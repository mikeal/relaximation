...
opts.addOption('-w', '--wclients', "number", "write_clients", 50, 
               "Number of concurrent write clients per process. Defaults to 50.");
opts.addOption('-r', '--rclients', "number", "read_clients", 200, 
               "Number of concurrent read clients per process. Defaults to 200.");
opts.addOption('-u', '--url1', "string", "url1", "http://localhost:5984", 
               "CouchDB url to run tests against. Defaults to http://localhost:5984");
opts.addOption('-v', '--url2', "string", "url2", "http://localhost:5985", 
               "CouchDB url to run tests against. Defaults to http://localhost:5985");
